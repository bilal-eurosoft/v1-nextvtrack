...
export default function Maintenance({maintenancedata, singleVehicleDetail}:any) { 
    ...
    useEffect(() => {
        const d = maintenancedata.filter((item) => item.vehicleId === singleVehicleDetail[0]._id);
          ...
          try {
            ...
            if (fetchedServices.length > 0) {
              const filteredServices = fetchedServices.filter(
                (service: any) => service.dataType === 'Maintenance' && service.vehicleId === singleVehicleDetail[0]._id
              );
          ...
          {(fetchedMaintencebyVehicle.length === 0) ? (
            ...
            fetchedMaintencebyVehicle.map((service, index) => (
              <tr key={service._id}>
                ...
                <td className="px-2 py-1">{service.maintenanceType}</td>
