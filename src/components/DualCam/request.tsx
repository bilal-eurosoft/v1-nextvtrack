...
import { portalGprsCommand, responsegprs, vehicleListByClientId, videoList, vehiclebyClientid, getGprsCommandLatest } from "@/utils/API_CALLS";
...
export default function Request({socketdata,deviceCommandText}) {
  ...
  useEffect(() => {
    if (latestGprs == true) {
      if (session) {
        setTimeout(async () => {
          const response = await getGprsCommandLatest({ token: session?.accessToken })
